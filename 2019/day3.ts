...
function tracePath(input: string[]): Point[] {
    let currPoint: Point = new Point(0, 0);
    let directions = {
        'R': new Point(1, 0), 'L': new Point(-1, 0),
        'U': new Point(0, 1), 'D': new Point(0, -1)
    };
    let path: Point[] = [currPoint];
    input.forEach(el => {
        let dir: string = el[0]
        let mag: number = parseInt(el.slice(1));
        currPoint = currPoint.add(directions[dir].scale(mag));
        path.push(currPoint);
    })
    return path
}

function findIntersectionPoint(p1: Point, p2: Point, p3: Point, p4: Point): Point {
    let t: number = (p1.x - p3.x) * (p3.y - p4.y) - (p1.y - p3.y) * (p3.x - p4.x)
    let u: number = -((p1.x - p2.x) * (p1.y - p3.y) - (p1.y - p2.y) * (p1.x - p3.x))
    let denom: number = (p1.x - p2.x) * (p3.y - p4.y) - (p1.y - p2.y) * (p3.x - p4.x)
    let out: Point;

    if ((0 <= t && t <= denom) && (0 <= u && u <= denom) || (denom <= t && t <= 0 && denom <= u && u <= 0)) {
        t /= denom;
        out = new Point(p1.x + t * (p2.x - p1.x), p1.y + t * (p2.y - p1.y));
    } else {
        out = null;
    }
    return out;
...
function passesOver(prev1: Point, curr1: Point, prev2: Point, curr2: Point): Point {
    let vert1: boolean = prev1.x === curr1.x;
    let vert2: boolean = prev2.x === curr2.x;

    if ((vert1 && !vert2) || (vert2 && !vert1)) {
        return findIntersectionPoint(prev1, curr1, prev2, curr2);
    }
...
function getIntersectionPoints(wire1: Point[], wire2: Point[]): Point[] {

    let intersectionPoints: Point[] = [];
    for (let i = 2; i < wire1.length; i++) {
        let prev1 = wire1[i - 1];
        let curr1 = wire1[i];
        for (let j = 1; j < wire2.length; j++) {
            let prev2 = wire2[j - 1];
            let curr2 = wire2[j];
            let iP: Point = passesOver(prev1, curr1, prev2, curr2);
            if (iP) intersectionPoints.push(iP);
        }
    }
...
function findMinDist(intersectionPoints: Point[]): number {
    let minDist: number = 1e6;
    intersectionPoints.forEach((el) => {
        let newDist: number = Math.abs(el.x) + Math.abs(el.y);
        minDist = newDist < minDist ? newDist : minDist;
    })
    return minDist;
}

function testSuite(wires: string, target1: number, target2: number): [boolean, boolean] {
    let array: string[] = wires.split("\n");
    let paths = [array[0].split(","), array[1].split(",")];

    let wire1 = tracePath(paths[0]);
    let wire2 = tracePath(paths[1]);
    let iPts: Point[] = getIntersectionPoints(wire1, wire2);
    let out: [boolean, boolean] = [false, false];

    out[0] = findMinDist(iPts) === target1;
    out[1] = part2(paths[0], paths[1], iPts) === target2;
    return out;
}


function part2(path1: string[], path2: string[], intersectionPoints: Point[]): number {
    // for each intersection point want to find the distance along the path to it;
    let distances = new Map();
    intersectionPoints.forEach((ip) => {
        distances.set(ip.toString(), [1e6, 1e6]);
    })
    function countSteps(path: string[], distances, index: number): any {
        ...
        path.forEach((el) => {
            ...
            let mag: number = parseInt(el.slice(1));
            for (let i = 0; i < mag; i++) {
                currPoint = currPoint.add(directions[dir]);
                d++;
                let currString: string = currPoint.toString();
                if (distances.has(currString)) {
                    distances.get(currString)[index] = d;
                }
            }
        })
        return distances;
    }
    distances = countSteps(path1, distances, 0);
    distances = countSteps(path2, distances, 1);

    let minDist = 2e6;
    distances.forEach((pair) => {
        let newDist = pair[0] + pair[1];
...
function main(): void {
    ...
    let intersectionPoints = getIntersectionPoints(wire1, wire2);
    console.log(`Part 1: ${findMinDist(intersectionPoints)}`)
    console.log(part2(paths[0], paths[1], intersectionPoints));
}
...
function tests(): void {
    console.log(testSuite(babyCase, 6, 30));
    console.log(testSuite(testInputs[0], 159, 610));
    console.log(testSuite(testInputs[1], 135, 410));
}
