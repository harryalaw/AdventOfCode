...
function findIntersectionPoint(p1, p2, p3, p4) {
    var t = (p1.x - p3.x) * (p3.y - p4.y) - (p1.y - p3.y) * (p3.x - p4.x);
    var u = -((p1.x - p2.x) * (p1.y - p3.y) - (p1.y - p2.y) * (p1.x - p3.x));
...
function passesOver(prev1, curr1, prev2, curr2) {
    var vert1 = prev1.x === curr1.x;
    var vert2 = prev2.x === curr2.x;
...
function getIntersectionPoints(wire1, wire2) {
    ...
    for (var i = 2; i < wire1.length; i++) {
        var prev1 = wire1[i - 1];
        var curr1 = wire1[i];
        for (var j = 1; j < wire2.length; j++) {
            var prev2 = wire2[j - 1];
            var curr2 = wire2[j];
            var iP = passesOver(prev1, curr1, prev2, curr2);
            if (iP)
...
function findMinDist(intersectionPoints) {
    var minDist = 1e6;
    intersectionPoints.forEach(function (el) {
        var newDist = Math.abs(el.x) + Math.abs(el.y);
        minDist = newDist < minDist ? newDist : minDist;
    });
    return minDist;
}
function testSuite(wires, target1, target2) {
    var array = wires.split("\n");
    var paths = [array[0].split(","), array[1].split(",")];
    var wire1 = tracePath(paths[0]);
    var wire2 = tracePath(paths[1]);
    var iPts = getIntersectionPoints(wire1, wire2);
    var out = [false, false];
    out[0] = findMinDist(iPts) === target1;
    out[1] = part2(paths[0], paths[1], iPts) === target2;
    return out;
}
function part2(path1, path2, intersectionPoints) {
    // for each intersection point want to find the distance along the path to it;
    var distances = new Map();
    intersectionPoints.forEach(function (ip) {
        distances.set(ip.toString(), [1e6, 1e6]);
    });
    function countSteps(path, distances, index) {
        var directions = {
            'R': new Point(1, 0), 'L': new Point(-1, 0),
            'U': new Point(0, 1), 'D': new Point(0, -1)
        };
        ...
        path.forEach(function (el) {
            ...
            for (var i = 0; i < mag; i++) {
                ...
                d++;
                var currString = currPoint.toString();
    ...
    var minDist = 2e6;
    distances.forEach(function (pair) {
        var newDist = pair[0] + pair[1];
...
function main() {
    ...
    var wire1 = tracePath(paths[0]);
    var wire2 = tracePath(paths[1]);
    var intersectionPoints = getIntersectionPoints(wire1, wire2);
